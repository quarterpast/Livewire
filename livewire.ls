...
module.exports = (routes = [])->
	function route method then (path,funcs)->
		...
		[]+++funcs |> concat-map (<<< do
			...
			handle: (req,res)->
				...
				(req <<< status-code:200){}params <<< if params? then tail vals |> zip that |> list-to-obj else vals
	...
	(http.create-server (req,res)->sync do
		:fiber -> let start = Date.now!, end$ = res.end
			res <<< end:(-> console.log "#{res.status-code} #{req.url}: #{Date.now! - start}ms"; end$ ...);
			req <<< (url.parse req.url,yes) <<< status-code:404
